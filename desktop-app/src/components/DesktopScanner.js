...
const DesktopScanner = ({ initialFolder, onDisplayFolder, onSwitchTab, disableResumeCheck = false }) => {
  ...
  const [previewLoadMode, setPreviewLoadMode] = useState('all'); // Always load all previews for folder scan
    ...
    if (shouldUseBatch) {
      ...
      if (batchResults && batchResults.length > 0) {
        ...
        if (isCriticalError(batchResults)) {
          const shouldStop = handleError('DesktopScanner', batchResults, setIsScanning);
          if (shouldStop) {
            setProcessing(false);
  ...
  const scanChildFolder = async (childPath) => {
    ...
    const shouldUseBatch = (
      isGeminiEngine && // Gemini engine (including Tesseract+Text)
      (batchMode === 'fixed' || batchMode === 'smart') && // Batch mode enabled
      files.length >= 2 // At least 2 files
    );
    ...
    if (shouldUseBatch) {
      ...
      if (batchResults && batchResults.length > 0) {
        ...
        // Check for critical errors using centralized handler
        if (isCriticalError(batchResults)) {
          const shouldStop = handleError('DesktopScanner - Folder', batchResults, setIsScanning);
          if (shouldStop) {
            return;
          }
        }
                ...
                <div key={t.path}>
                  ...
                  <div className="mt-6 pt-4 border-t flex items-center justify-between">
                    ...
                    <div className="text-sm text-gray-600">
                      Tab {childTabs.findIndex(tab => tab.path === activeChild) + 1} / {childTabs.length}
                    </div>
